...
### Chengry
You now have two paths for medical AI: the specialist (biomedical NER, free on CPU) and the generalist (Claude API). This week, push the specialist. Test the NER model on different kinds of medical text -- clinical notes, patient-facing health advice, drug labels, research abstracts. Where does it extract entities well? Where does it miss? Does it find false entities in non-medical text? Document the edge cases in your journal -- that boundary where the specialist stops being reliable is external validity for a domain-specific model. Then think about your project: could your DxAI tool combine both paths -- a free NER model for structured entity extraction and an API call for conversational explanation? That's a real architecture used in production medical AI systems.
